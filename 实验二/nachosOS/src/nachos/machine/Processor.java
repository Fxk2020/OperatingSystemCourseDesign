 ...
 * 
 * <tt> Processor </ tt>类模拟支持R3000指令集的子集的MIPS处理器。 
 * 具体来说，处理器缺乏所有协处理器支持，只能在用户模式下执行。 
 * 地址转换信息可通过API访问。 
 * 该API还允许内核设置要在任何用户模式异常下调用的异常处理程序。
 * 
 * 
 * nachos进行内存管理的类：memory management unit，缩写为MMU
 *
 * <p>
...
public final class Processor {
     ...
     *
     * 返回对物理内存阵列的引用。 
     * 该数组的大小为<tt> pageSize * getNumPhysPages（）</ tt>。
     *
    ...
    public static final int pageSize = 0x400;//大小是1024
